...
$fi.fn.Livre=create_class(
	function(o){
		...
		toJSON:function(){ return { parents:this.parents, date:this.date, author:this.author, message:this.message, accounts:this.accounts, }; },
		toVSON:function(){ r = this.toJSON(); r.accounts=this.lod('accounts').toVSON(); return r; },
		...
		diff:function(id){
			...
			return new this.$.Ecriture( { mouvement: this.lod('accounts').diff(old.lod('accounts')).sort(function(a,b){
				if( a[1] > b[1]	) return 1;
				if( b[1] > a[1]	) return -1;
